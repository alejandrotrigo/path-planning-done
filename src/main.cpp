...
const double MIN_DIST = SECURITY_DISTANCE / 2;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		for (auto sf : sensor_fusion){
			...
			// If car in the other waylanes continue
			...
			if (check_lane == lane){
				...
				//There is a car two lines right or left, we dont care since we are not going to change 2 lines at onece
			}	
		}
            ...
            //Later we'll interpolate this waypoints with a spline and fill it in with more points that control speed.
            vector<double> ptsx;
            vector<double> ptsy;
            ...
            //In frenet add evenly 30m spoced points ahead of the starting reference
